...
Para utilizar o `ggplot2` juntamente com o `tidyverse`, basta importar a biblioteca `tidyverse`, que já inclui o `ggplot2`.
...
E para o gráfico de barras usamos a função `geom_bar()`.  No entanto, essa função cria uma contagem dos dados, então devemos atribuir apenas uma estética `x` ou `y`.
...
Então geom_bar é realmente um atalho para:
...
Também podemos utlizar para verificar a função de distribuição dos dados. Vamos testar uma função de distribuição normal, pela função `stat_function`.
...
dados %>%
  ...
  stat_function(
    fun = dnorm, 
    aes(colour = "red"), 
    args = list(mean = mean(dados$area), sd = sd(dados$area))
  )
```
...
dados %>%
  ...
  geom_vline(xintercept = mean(dados$area), colour = "green", linetype = "longdash")
```

Agora podemos considerar apenas quais `area` apresentam `gen` com contagem acima de 10.
...
Para gráficos de correlação precisamos antes de plotar o gráfico calcular a correlação entre as variáveis. 

Vamos estimar a correlação das variaveis `sev1`, `sev2`, `sev3`, `area` e `areat`. 

Usaremos a função `corr_coef()`, do pacote `metan` [(mais informações sobre esse pacote)](https://tiagoolivoto.github.io/metan/index.html) para retornar uma matriz de correlação das nossas variáveis e a função `plot()` para plotar o gráfico. 
...
Para produzir um gráfico de regressão utilizamos a função `geom_smooth()`. Por padrão, essa função utiliza o método não-paramétrico "loess" e a fórmula "y ~ x". 

Nesse gráfico, podemos definir os argumentos `method =`, que define qual método iremos utilizar, por exemplo `method =lm` para definir uma regressão linear entre `x` e `y`, `se=` que é um argumento lógico (ou seja, recebe `TRUE` ou `FALSE`) e indica se será representado o intervalo de confiança.
...
ggplot(data = dados, aes(x = id, y = area)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
...
As facetas são utilizadas para criar gráficos para cada categoria de uma variável. Temos duas Podemos utilizar as funções  `facet_grid()` ou `facet_wrap()` para criar essas facetas.
