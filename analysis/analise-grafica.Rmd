...
### Gráfico de barras com alteração da posição da legenda
...
### Gráfico com 3 eixos - Barras e linha
...
ggplot() +
  # plot the bar chart
  geom_bar(data = data,
           aes(as.factor(Year), fill = Growth_type),
           width = 0.8) +
  # plot the line chart
  geom_line(
    data = yield,
  ...
  # plot the points on the line chart
  geom_point(
    data = yield,
    aes(x = Year, y = (Yield * (175 / 125000))),
    group = 1,
  ...
  # add a secondary y axis for the yield
  scale_y_continuous(name = "Count",
                     sec.axis = sec_axis(~ . * 125000 / 175, name = "Yield (Thousand Tons)")) +
  # set the fill color palette
  scale_fill_brewer(palette = "Dark2") +
  # set the labels for the legend and x-axis
  labs(fill = "Event Type", x = "Year") +
  # set the theme to a classic style
  theme_classic() +
  # customize the legend and axis text styles
  theme(
...
dados %>%
  ...
  geom_text(
    aes(y = mean + sd, label = group),
    position = position_dodge(width = 0.85),
    vjust = -0.5,
    size = 4
  ) +
  scale_fill_gdocs() +
  labs(y = "Produtividade", x = "") +
  theme_classic() +
  theme(
